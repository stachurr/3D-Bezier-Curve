...
#ifdef WIN32
#include <windows.h>
#pragma warning(disable:4996 26451 4305 4244)
#endif
...
//
//	Author:			Ryan Stachura

// bunch o' stuff
// title of these windows:

const char *WINDOWTITLE = { "OpenGL / Project 6 - Geometric Modeling, Ryan Stachura" };
...
//void	DoShadowMenu();
void	DoRasterString( float, float, float, std::string );
...
// Project 6
const int	MS_PER_ANIMATION_CYCLE	= 5 * 1000;	// milliseconds in the animation loop
Bezier		mainCurve;
GLuint		RangeOutline;
...
main( int argc, char *argv[ ] )
{
	...
	// main curve (RANDOM) initialization
	{
		int DEGREE = 7;

		for (int i = 0; i < DEGREE + 1; i++) mainCurve.addPoint(0.0, 0.0, 0.0);
		mainCurve.randomize();
	}

	// timers
	Timer randomizeMainCurve;
	randomizeMainCurve.setInterval([&]() {
		mainCurve.randomizeAndInterpolate(0.8);
	}, 5000);
...
Animate( )
{
	...
	ms	   %= MS_PER_ANIMATION_CYCLE;		// milliseconds in the range 0 to MS_PER_ANIMATION_CYCLE-1
	Time	= (float)ms / (float)MS_PER_ANIMATION_CYCLE;	// [ 0., 1. )
...
{
	// set up
		if( DebugOn != 0 ) fprintf( stderr, "Display\n" );
	...
	// Set look-at
	gluLookAt(
		0.0, 0.0, 20.0,	//mainCurve.getControlPoints().getRange() * 2.5,	// eye pos
	...
	// randomized and interpolated scene
	{
	// Range outline box
	{
		glPushMatrix();
			glCallList(RangeOutline);
			glRotated(90, 1, 0, 0);
			glCallList(RangeOutline);
			glRotated(90, 1, 0, 0);
			glCallList(RangeOutline);
...
void
DoControlLinesMenu( int id )
{
	mainCurve.showControlLines((bool)id);
...
void
DoControlPointsMenu( int id )
{
	mainCurve.showControlPoints((bool)id);
...
void
DoRasterString( float x, float y, float z, std::string s )
{
...
InitLists( )
{
	...
	float r = mainCurve.getControlPoints().getRange();
	RangeOutline = glGenLists(1);
	glNewList(RangeOutline, GL_COMPILE);
...
Reset( )
{
	...
	// Project 6
	mainCurve.showControlLines(false);
	mainCurve.showControlPoints(false);
}
